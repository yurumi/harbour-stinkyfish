...
class OrgSchedule(OrgPlugin):
    ...
    class Element(OrgElement):
        ...
        def __init__(self, scheduled=None, deadline=None, closed=None):
...
class OrgList(OrgPlugin):
    """A plugin for list management"""
    def __init__(self):
        OrgPlugin.__init__(self)
        self.regexp = re.compile(r"^\s*\-")

    def _treat(self, current, line):
        item = self.regexp.match(line)
        if item:
            if not isinstance(current, self.Element):
                current = current.append(self.Element())
    ...
    class Element(OrgElement):
        ...
        A list item object
        """
        TYPE = "LIST_ITEM_ELEMENT"

        def __init__(self):
            OrgElement.__init__(self)

        def _output(self):
            output = f"- {self.content[0]}\n"
            return output
...
class OrgNode(OrgPlugin):
    ...
    class Element(OrgElement):
        ...
        def get_all_tags(self, use_tag_inheritance=True,
                         tags_exclude_from_inheritance=None):
            ...
            if tags_exclude_from_inheritance is None:
...
class OrgKeywords(OrgPlugin):
    """A plugin that recognizes KEYWORDS & adds them to the root object."""

    IN_BUFFER_RE = re.compile(r'\s*#\+TITLE:\s+(.*)', re.I)
...
class OrgDataStructure(OrgElement):
    ...
    @staticmethod
    def get_nodes_by_priority(node, priority, found_nodes=None):
    ...
    @staticmethod
    def get_node_by_heading(node, heading, found_nodes=None):
        if found_nodes is None:
            found_nodes = []
