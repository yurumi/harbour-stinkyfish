...
BaseNode {
    ...
    function update() {
        if(mode == 1){
            statusAuto = calcChildrenProgress(nodeId)
        } // mode == 1
    }

    function calcChildrenProgress(parentNodeId){
        var ret = 0
        var children = Database.getChildNodes(parentNodeId)
        var sumProgress = 0
        var numTodoNodes = 0
        ...
        if(parentNodeId == baseNode.nodeId){
